...
describe('context route', () => {
  ...
  it('?history=full returns the pre-compaction messages (full reconstructed history)', async () => {
    ...
    // assistant reply is dropped, then the summary, the continue-reminder, then
    ...
    // is KEPT, then the summary marker, the continue-reminder, then the
    ...
    expect(fullBody.messages.map((m) => m.source)).toEqual([
      'append_message', 'append_message', 'compaction_summary', 'append_message', 'append_message',
    ]);
    ...
    expect(fullBody.messages[4]!.message.content[0]).toMatchObject({ text: 'after compaction' });
  });
});
