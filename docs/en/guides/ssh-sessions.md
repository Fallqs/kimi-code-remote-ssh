...
The CLI also reconnects in the background. A re-established pipe waits for one acknowledgment before carrying calls again, and the next remote tool call counts as that acknowledgment — it proceeds on the new pipe instead of failing, so an agent retrying after a blip recovers on its own. To acknowledge explicitly without running a tool call, run `/resume-remote` in the TUI, or call `POST /api/v1/workspaces/{id}/ssh/resume` over the REST API (the current state is readable from `GET /api/v1/workspaces/{id}/ssh/state`).
